...
parser.add_argument(
    "-o",
    "--output",
    help='Output file (defaults to "cmp-outages.png")',
...
parser.add_argument(
    "-w",
    "--width",
    help='Width in "inches", depending on dpi (defaults to 12.0)',
    default=12.0,
    type=float,
    action="store",
)
...
bbox_group.add_argument(
    "-e", "--entirestate", help="Show entire state", action="store_true"
)
bbox_group.add_argument(
    "-z",
    "--zoom",
    help="Zoom to only this town (mupltiple allowed)",
    metavar="TOWN",
    default=[],
    action="append",
)
parser.add_argument(
    "-i",
    "--interesting",
    help="Highlight this town (mupltiple allowed)",
    metavar="TOWN",
    default=[],
    action="append",
)
...
if not Path("Maine_E911_NG_Roads/Maine_E911_NG_Roads.shp").exists():
    if Path("Maine_E911_NG_Roads.zip").exists():
        ...
        zipfile.ZipFile("Maine_E911_NG_Roads.zip", "r").extractall(
            "Maine_E911_NG_Roads"
        )
        if not args.quiet:
            print("...done.")
        ...
        print(
            "ERROR: no .ship file in Maine_E911_NG_Roads/Maine_E911_NG_Roads.shp and no zip to unpack"
        )
        sys.exit()
...
try:
    for shapeRec in allRoadIterator:
        ...
        if not (args.fast and color == "black" and speed < 36):
            line = plt.Polygon(
